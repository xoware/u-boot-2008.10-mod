...
int enter_MX_25L256_4byte_mode(void);
int exit_MX_25L256_4byte_mode(void);
...
Spi_Flash_Read_Data_Bytes(u8 spi_flash_channel, u32 address, u8 * read_buffer, u32 len)
{
	...
	//printf("len: %d ## address: %x\n", len, address);
	// current is for MX_25L256
	// Note the sector address is 32-Bit
	// write 24:32 bit address part
	if (address >= 0x1000000) {
		if (spi_flash_bank[0].device_id == MX_25L256)
			enter_MX_25L256_4byte_mode();
	}
	// Wait until Flash is ready
	while (!Spi_Flash_Is_Flash_Ready(spi_flash_channel)) ;
...
#endif
	if (address >= 0x1000000) {
		Spi_Flash_Buffer_Transmit_Receive(spi_flash_channel, 0, (u32) ((address >> 24) & 0xFF), &rx_data);
	}
...
Spi_Flash_Sector_Erase(u8 spi_flash_channel, u32 sector_addr)
{
	...
	if (Spi_Flash_Set_Write_Enable(spi_flash_channel)) {
		...
		if (sector_addr >= 0x1000000) {
			Spi_Flash_Buffer_Transmit_Receive(spi_flash_channel, 0, (u32) ((sector_addr >> 24) & 0xFF), &rx_data);
		}
...
{ // enter 4-byte mode.
        ...
        u32 rx_data1, rx_data2, rx_data3;
	u8 spi_flash_channel=0;
         
        // Wait until Flash is ready
        while (!Spi_Flash_Is_Flash_Ready(spi_flash_channel)) ;
             
        Spi_Flash_Buffer_Transmit_Receive(spi_flash_channel, 1, 0xb7, &rx_data1);
	return 0;
}
...
{ // exit 4-byte mode.
	...
	u8 spi_flash_channel=0;
        ...
        Spi_Flash_Buffer_Transmit_Receive(spi_flash_channel, 1, 0xe9, &rx_data1);
	return 0;
}
...
int AT91F_DataflashProbe(int cs, AT91PS_DataflashDesc pDesc)
{
	...
	switch ((manufacturer_id << 16) | device_id) {
        case W25X20B:		
		spi_flash_bank[0].sectors = WINBOND_W25X20B_SECTOR_NUM;
		spi_flash_bank[0].sector_size = SPI_FLASH_SECTOR_SIZE;
		spi_flash_bank[0].pages = WINBOND_W25X20B_PAGE_NUM;
	...
	case MX_25L128:
		...
		break;
	case MX_25L256:
		area_list[0].end = 0x01FFFFFF;
		spi_flash_bank[0].device_id = MX_25L256;
		spi_flash_bank[0].sectors = MX_25L256_SECTOR_NUM;
		spi_flash_bank[0].sector_size = SPI_FLASH_SECTOR_SIZE;
		spi_flash_bank[0].pages = MX_25L256_PAGE_NUM;
		spi_flash_bank[0].page_size = SPI_FLASH_PAGE_SIZE;
		break;
	case EN_25P32:
		...
		break;
        case SP_S25FL128P:
                spi_flash_bank[0].sectors = SP_S25FL128P_SECTOR_NUM;
                spi_flash_bank[0].sector_size = SPI_FLASH_SECTOR_SIZE;
                spi_flash_bank[0].pages = SP_S25FL128P_PAGE_NUM;
                spi_flash_bank[0].page_size = SPI_FLASH_PAGE_SIZE;
                break;
        case SP_S25FL064A:
                spi_flash_bank[0].sectors = SP_S25FL064A_SECTOR_NUM;
                spi_flash_bank[0].sector_size = SPI_FLASH_SECTOR_SIZE;
                spi_flash_bank[0].pages = SP_S25FL064A_PAGE_NUM;
                spi_flash_bank[0].page_size = SPI_FLASH_PAGE_SIZE;
                break;
        case NX_N25Q064:
                spi_flash_bank[0].sectors = NX_N25Q064_SECTOR_NUM;
                spi_flash_bank[0].sector_size = SPI_FLASH_SECTOR_SIZE;
                spi_flash_bank[0].pages = NX_N25Q064_PAGE_NUM;
                spi_flash_bank[0].page_size = SPI_FLASH_PAGE_SIZE;
                break;
	default:
		break;
...
#else
	/* 
	 * fix last sector access bug, 
	 * we caculate start/end sector by address directly
	 */
	/* due to that dest && size are already verified on upper layer,
	 * sainty check is unnecessary here 
 	 */
	start_sector = dest / spi_flash_info->sector_size;;
	end_sector = (dest + size - 1) / spi_flash_info->sector_size;
#endif
...
#else
flash_info_t s_flash_info[CFG_MAX_FLASH_BANKS];    /* info for FLASH chips */
static ulong spi_flash_get_size(vu_long *addr, flash_info_t *info);
#endif
...
#else
		s_flash_info[i].flash_id = FLASH_UNKNOWN;
#endif
...
#else
	size = spi_flash_get_size((vu_long *)CFG_SPI_FLASH_BASE, &s_flash_info[0]);
	s_flash_info[0].size = size;
#endif
...
#endif
{
	...
	switch ((manufacturer_id << 16) | device_id) {
        case W25X20B:		
		printf("Flash Manufacturer: WINBOND\n");
		spi_flash_bank[0].sectors = WINBOND_W25X20B_SECTOR_NUM;
		spi_flash_bank[0].sector_size = SPI_FLASH_SECTOR_SIZE;
		spi_flash_bank[0].pages = WINBOND_W25X20B_PAGE_NUM;
		spi_flash_bank[0].page_size = SPI_FLASH_PAGE_SIZE;
	...
	case MX_25L128:
		...
		spi_flash_bank[0].pages = MX_25L128_PAGE_NUM;
		spi_flash_bank[0].page_size = SPI_FLASH_PAGE_SIZE;
		info->flash_id = FLASH_MAN_MX + MX_25L128_DEVICE_ID;
		info->sector_count = 256;
		info->size = 0x1000000; /* total bank size in bytes */
	...
	case MX_25L256:
		...
		spi_flash_bank[0].pages = MX_25L256_PAGE_NUM;
		spi_flash_bank[0].page_size = SPI_FLASH_PAGE_SIZE;
		info->flash_id = FLASH_MAN_MX + MX_25L256_DEVICE_ID;
		info->sector_count = 512;
		info->size = 0x2000000; /* total bank size in bytes */
		memset(info->protect, 0, 512);
		for (i = 0; i < info->sector_count; i++) {
			info->start[i] = base;
			base += 0x10000;
		}
		break;
	//If you add one SPI flash, you need also add that flash's infomation in drivers/mtd/dataflash.c ...
	case MX_25L4006E:
		printf("Flash Manufacturer: MX\n");
		spi_flash_bank[0].sectors = MX_25L4006E_SECTOR_NUM;
		spi_flash_bank[0].sector_size = SPI_FLASH_SECTOR_SIZE;
		spi_flash_bank[0].pages = MX_25L4006E_PAGE_NUM;
		spi_flash_bank[0].page_size = SPI_FLASH_PAGE_SIZE;
		info->flash_id = FLASH_MAN_MX + MX_25L4006E_DEVICE_ID;
		info->sector_count = 128;
		info->size = 0x80000; /* total bank size in bytes */
		memset(info->protect, 0, 128);
		for (i = 0; i < info->sector_count; i++) {
			info->start[i] = base;
			base += 0x1000;
	...
	// Added by FAE                
        case SP_S25FL128P:
                ...
                spi_flash_bank[0].pages = SP_S25FL128P_PAGE_NUM;
                spi_flash_bank[0].page_size = SPI_FLASH_PAGE_SIZE;
                info->flash_id = FLASH_MAN_STM + SP_S25FL128P_DEVICE_ID;
                info->sector_count = 256;
                info->size = 0x1000000;
                memset(info->protect, 0, 256);
                for (i = 0; i < info->sector_count; i++) {
                ...
                break;                
        case NX_N25Q064:
                printf("Flash Manufacturer: Numonyx\n");
                spi_flash_bank[0].sectors = NX_N25Q064_SECTOR_NUM;
                spi_flash_bank[0].sector_size = SPI_FLASH_SECTOR_SIZE;
                spi_flash_bank[0].pages = NX_N25Q064_PAGE_NUM;
                spi_flash_bank[0].page_size = SPI_FLASH_PAGE_SIZE;
                info->flash_id = FLASH_MAN_STM + NX_N25Q064_DEVICE_ID;
                info->sector_count = 128;
                info->size = 0x800000;
                memset(info->protect, 0, 128);
                for (i = 0; i < info->sector_count; i++) {
...
#else
int flash_erase_spi(flash_info_t *info, int s_first, int s_last)
#endif
{
	struct spi_flash_info *spi_flash_info = &spi_flash_bank[0];
	int flag, prot, sect;
