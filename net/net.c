...
NetReceive(volatile uchar * inpkt, int len)
{
...
#ifdef CONFIG_UDP_FRAGMENT
	uint32_t ip_off;	/* ip_off for fragmentation */
#endif
	...
	case PROT_IP:
		 ...
		 */
		ip_off = (ntohs(ip->ip_off) & IP_OFFS) << IP_OFFS_SHIFT;
		if ((ip_off != 0) || (ip->ip_off & htons(IP_FLAGS_MFRAG))) {
			...
			/* Copy the UDP hdr with the data for 1st fragment,
			 * else just copy payload.
			 */
			if (ip_off == 0) {
			...
			memcpy(NetFragBuf + ip_off, start, size);

			/*
			 * When last fragment has been received we
...
#endif /* CONFIG_UDP_CHECKSUM */
			(*packetHandler)(NetFragBuf + 8, udp_dst, udp_src, udp_len - 8);
