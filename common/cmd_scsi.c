...
void swap_ident(unsigned char *src, unsigned int len);
	...
	for(i=0;i<CFG_SCSI_MAX_SCSI_ID;i++) {
		...
		for(lun=0;lun<CFG_SCSI_MAX_LUN;lun++) {
			...
			if (ataid[pccb->target]->command_set_2 & COMMANDSET2_LBA48) {
				printf("Supprt LBA48 addressing.\n");
				scsi_dev_desc[scsi_max_devs].lba48 = 1;
			} else {
				printf("Not supprt LBA48 addressing.\n");
				scsi_dev_desc[scsi_max_devs].lba48 = 0;
			}
			capacity=((lbaint_t)tempbuff[0]<<56)|((lbaint_t)tempbuff[1]<<48)|
					((lbaint_t)tempbuff[2]<<40)|((lbaint_t)tempbuff[3]<<32)|
					((lbaint_t)tempbuff[4]<<24)|((lbaint_t)tempbuff[5]<<16)|
					((lbaint_t)tempbuff[6]<<8)|((lbaint_t)tempbuff[7]);
			blksz=((unsigned long)tempbuff[8]<<24)|((unsigned long)tempbuff[9]<<16)|
				((unsigned long)tempbuff[10]<<8)|((unsigned long)tempbuff[11]);
#else
...
U_BOOT_CMD(
	...
	"init  - initialize SCSI controller\n"
