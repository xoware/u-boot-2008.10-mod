...
extern flash_info_t s_flash_info[CFG_MAX_FLASH_BANKS];
#endif
/*
...
{
	int ret=0;
	if(info->flash_id == s_flash_info[0].flash_id)
		ret = flash_erase_spi(info, s_first, s_last);
	else    
		ret = flash_erase_cfi(info, s_first, s_last);
	return ret;
}
#endif
...
#ifdef CONFIG_SUPPORT_PFLASH_AND_SFLASH
int do_flerase_spi (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	flash_info_t tmp;
	int ret=0;

	memcpy(&tmp, &flash_info[0], sizeof tmp);
	memcpy(&flash_info[0], &s_flash_info[0], sizeof tmp);
	ret = do_flerase(cmdtp, flag, argc, argv);
	memcpy(&s_flash_info[0], &flash_info[0], sizeof tmp);
	memcpy(&flash_info[0], &tmp, sizeof tmp);
	return ret;
}
#endif
...
#ifdef CONFIG_SUPPORT_PFLASH_AND_SFLASH
int do_protect_spi (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	flash_info_t tmp;
	int ret=0;
	...
	memcpy(&tmp, &flash_info[0], sizeof tmp);
	memcpy(&flash_info[0], &s_flash_info[0], sizeof tmp);
	ret = do_protect(cmdtp, flag, argc, argv);
	memcpy(&s_flash_info[0], &flash_info[0], sizeof tmp);
	memcpy(&flash_info[0], &tmp, sizeof tmp);
	return ret;
}
...
U_BOOT_CMD(
...
#else
	"    - erase parallel FLASH from addr 'start' to addr 'end'\n"
#endif
	"erase start +len\n"
#ifndef CONFIG_SUPPORT_PFLASH_AND_SFLASH
	"    - erase FLASH from addr 'start' to the end of sect "
#else
	"    - erase parallel FLASH from addr 'start' to the end of sect "
#endif
	"w/addr 'start'+'len'-1\n"
#ifndef CONFIG_SUPPORT_PFLASH_AND_SFLASH
	...
	"erase bank N\n    - erase FLASH bank # N\n"
#else
	"erase N:SF[-SL]\n    - erase sectors SF-SL in parallel FLASH bank # N\n"
	"erase bank N\n    - erase parallel FLASH bank # N\n"
#endif
	TMP_ERASE
#ifndef CONFIG_SUPPORT_PFLASH_AND_SFLASH
	"erase all\n    - erase all FLASH banks\n"
#else
	"erase all\n    - erase all parallel FLASH banks\n"
#endif
);

#ifdef CONFIG_SUPPORT_PFLASH_AND_SFLASH
U_BOOT_CMD(
	erase_spi,   3,   0,  do_flerase_spi,
	"erase_spi   - erase FLASH memory via serial interface\n",
	"start end\n"
	"    - erase serial FLASH from addr 'start' to addr 'end'\n"
	"erase_spi start +len\n"
	"    - erase serial FLASH from addr 'start' to the end of sect "
	"w/addr 'start'+'len'-1\n"
	"erase_spi N:SF[-SL]\n    - erase sectors SF-SL in serial FLASH bank # N\n"
	"erase_spi bank N\n    - erase serial FLASH bank # N\n"
	TMP_ERASE
	"erase_spi all\n    - erase all serial FLASH banks\n"
);
#endif
...
#else
	"    - protect parallel FLASH from addr 'start' to addr 'end'\n"
#endif
	"protect on start +len\n"
#ifndef CONFIG_SUPPORT_PFLASH_AND_SFLASH
	"    - protect FLASH from addr 'start' to end of sect "
#else
	"    - protect parallel FLASH from addr 'start' to end of sect "
#endif
	"w/addr 'start'+'len'-1\n"
	"protect on  N:SF[-SL]\n"
#ifndef CONFIG_SUPPORT_PFLASH_AND_SFLASH
...
#else
	"    - protect sectors SF-SL in parallel FLASH bank # N\n"
	"protect on  bank N\n    - protect parallel FLASH bank # N\n"
#endif
	TMP_PROT_ON
#ifndef CONFIG_SUPPORT_PFLASH_AND_SFLASH
	"protect on  all\n    - protect all FLASH banks\n"
#else
	"protect on  all\n    - protect all parallel FLASH banks\n"
#endif
	"protect off start end\n"
#ifndef CONFIG_SUPPORT_PFLASH_AND_SFLASH
	"    - make FLASH from addr 'start' to addr 'end' writable\n"
#else
	"    - make parallel FLASH from addr 'start' to addr 'end' writable\n"
#endif
	"protect off start +len\n"
#ifndef CONFIG_SUPPORT_PFLASH_AND_SFLASH
	"    - make FLASH from addr 'start' to end of sect "
#else
	"    - make parallel FLASH from addr 'start' to end of sect "
#endif
...
#else
	"    - make sectors SF-SL writable in parallel FLASH bank # N\n"
	"protect off bank N\n    - make parallel FLASH bank # N writable\n"
#endif
	TMP_PROT_OFF
#ifndef CONFIG_SUPPORT_PFLASH_AND_SFLASH
	"protect off all\n    - make all FLASH banks writable\n"
#else
	"protect off all\n    - make all parallel FLASH banks writable\n"
#endif
);

#ifdef CONFIG_SUPPORT_PFLASH_AND_SFLASH
U_BOOT_CMD(
	protect_spi,  4,  0,   do_protect_spi,
	"protect_spi - enable or disable serial FLASH write protection\n",
	"on  start end\n"
	"    - protect serial FLASH from addr 'start' to addr 'end'\n"
	"protect_spi on start +len\n"
	"    - protect serial FLASH from addr 'start' to end of sect "
	"w/addr 'start'+'len'-1\n"
	"protect_spi on  N:SF[-SL]\n"
	"    - protect sectors SF-SL in serial FLASH bank # N\n"
	"protect_spi on  bank N\n    - protect serial FLASH bank # N\n"
	TMP_PROT_ON
	"protect_spi on  all\n    - protect all serial FLASH banks\n"
	"protect_spi off start end\n"
	"    - make serial FLASH from addr 'start' to addr 'end' writable\n"
	"protect_spi off start +len\n"
	"    - make serial FLASH from addr 'start' to end of sect "
	"w/addr 'start'+'len'-1 wrtable\n"
	"protect_spi off N:SF[-SL]\n"
	"    - make sectors SF-SL writable in serial FLASH bank # N\n"
	"protect_spi off bank N\n    - make serial FLASH bank # N writable\n"
	TMP_PROT_OFF
	"protect_spi off all\n    - make all serial FLASH banks writable\n"
);
#endif
